...
exports.addUser = functions.auth.user().onCreate((user) => {
    return admin.database().ref().child('users').child(user.uid).set({
        uid: user.uid,
...
exports.generateThumbnails = functions.storage.object().onFinalize((object) => {
    ...
    return bucket.file(filePath).download({
        ...
        const thumbFileName = `thumb_1000x1000_${thumbName}`;
        ...
        console.log('Thumbnail created at', destThumbFilePath);
        // We add a 'thumb_' prefix to thumbnails file name. That's where we'll upload the thumbnail.
        const thumbFileName = `thumb_200x200_${thumbName}`;
        const thumbFilePath = path.join(path.dirname(filePath), 'thumbs', thumbFileName);
        // Uploading the thumbnail.
        return bucket.upload(destThumbFilePath, {
            destination: thumbFilePath,
...
exports.addAboutImages = functions.storage.object().onFinalize((object) => {
    const filePath = object.name; // File path in the bucket.
    const contentType = object.contentType; // File content type.
    const mediaLink = object.mediaLink; // Link to access
    ...
    return admin.database().ref('about/images').child(name).child(size).set(mediaLink);
});